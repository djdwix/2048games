...
def cleanup_key_data():
    try:
        ...
        with KEY_LOCK:
            ...
            keys_to_delete = [key for key, key_info in key_data.get('keys', {}).items() if now - key_info.get('created_at', 0) > 300]
...
def cleanup_quota_data():
    try:
        ...
        with QUOTA_LOCK:
            quota_data = load_quota_data()
            keys_to_delete = [hour_key for hour_key in quota_data if now.date() != datetime.strptime(hour_key, '%Y-%m-%d %H:00').date()]
            for key in keys_to_delete:
                del quota_data[key]
            save_quota_data(quota_data)
    except Exception as e:
...
def rate_limit():
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            with request_lock:
                to_delete = [ip for ip, timestamp in recent_requests.items() if timestamp < cleanup_threshold]
                for ip in to_delete:
...
def before_request():
    ...
    if is_maintenance_time():
        if not request.path.startswith('/404.html') and not request.path.startswith('/api/health') and not request.path.startswith('/style.css'):
...
def serve_static(path):
    if is_maintenance_time():
        if path not in ['404.html', 'style.css']:
            return redirect('/404.html')
...
def check_key_quota(client_ip):
    ...
    with KEY_LOCK:
        key_data = load_key_data()
        if key_data.get('current_hour') != hour_key:
            key_data['requests'] = {}
...
def request_key():
    ...
    try:
        ...
        if not check_key_quota(client_ip):
            update_api_stats('request_key', False)
            return jsonify({'success': False, 'error': f'当前时段已申请3次密钥，请等待下一小时段'}), 429
        ...
        with KEY_LOCK:
            ...
            key_data['keys'][new_key] = {'ip': client_ip, 'created_at': created_at, 'used': False}
            if 'requests' not in key_data:
                key_data['requests'] = {}
            if client_ip not in key_data['requests']:
                key_data['requests'][client_ip] = []
            key_data['requests'][client_ip].append({'timestamp': created_at, 'key': new_key})
            save_key_data(key_data)
            update_api_stats('request_key', True)
            return jsonify({'success': True, 'key': new_key, 'expires_in': 300})
    except Exception as e:
        update_api_stats('request_key', False)
...
def verify_key():
    ...
    try:
        ...
        with KEY_LOCK:
            ...
            if key not in key_data.get('keys', {}):
                update_api_stats('verify_key', False)
                return jsonify({'success': False, 'error': '密钥不存在或已过期'}), 404
            key_info = key_data['keys'][key]
            if key_info['ip'] != client_ip:
                update_api_stats('verify_key', False)
                return jsonify({'success': False, 'error': '密钥与客户端IP不匹配'}), 403
            if time.time() - key_info['created_at'] > 300:
                update_api_stats('verify_key', False)
                return jsonify({'success': False, 'error': '密钥已过期'}), 400
            update_api_stats('verify_key', True)
            return jsonify({'success': True, 'message': '密钥验证成功', 'key': key})
    except Exception as e:
        update_api_stats('verify_key', False)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_ip_info():
    ...
    try:
        ...
        server_ip = get_server_ip()
        client_ip = get_client_ip()
        return jsonify({'success': True, 'server_ip': server_ip, 'client_ip': client_ip})
    except Exception as e:
        update_api_stats('get_ip_info', False)
        return jsonify({'success': False, 'error': str(e), 'server_ip': '获取失败', 'client_ip': '获取失败'}), 500
...
def generate_custom_number():
    ...
    try:
        ...
        if not quota_result:
            update_usage_stats(False)
            update_api_stats('generate_custom_number', False)
            return jsonify({'success': False, 'error': f'配额已用尽（当前时段已使用{current_count}/{QUOTA_LIMIT}次），请稍后再试'}), 429
        data = request.json or {}
        ...
        if custom_prefix and (len(custom_prefix) != 3 or not custom_prefix.isdigit()):
            update_usage_stats(False)
            update_api_stats('generate_custom_number', False)
            return jsonify({'success': False, 'error': '自定义号段必须是3位数字'}), 400
        if custom_prefix and not is_valid_custom_prefix(custom_prefix):
            update_usage_stats(False)
            update_api_stats('generate_custom_number', False)
            return jsonify({'success': False, 'error': '自定义号段无效或未被分配运营商，请使用有效的号段'}), 400
        conn = get_db_connection()
        cursor = conn.cursor()
        max_attempts = 10
        for _ in range(max_attempts):
            ...
            try:
                cursor.execute(
                    '''INSERT INTO phone_numbers (phone_number, client_ip, carrier, purpose, request_id, custom_prefix)
                       VALUES (?, ?, ?, ?, ?, ?)''',
                    (phone_number, client_ip, carrier, purpose, request_id, custom_prefix if custom_prefix else None)
                )
                conn.commit()
...
def generate_security_code_for_number():
    ...
    try:
        ...
        if not result:
            update_api_stats('generate_security_code_for_number', False)
            return jsonify({'success': False, 'error': '手机号不存在'}), 404
        if result['is_used']:
            update_api_stats('generate_security_code_for_number', False)
        ...
        if result['security_code']:
            if security_code_expires_at and now < security_code_expires_at:
                if result['client_ip'] != client_ip:
                    update_api_stats('generate_security_code_for_number', False)
                    return jsonify({'success': False, 'error': '安全码只能在生成时的客户端下使用'}), 403
                update_api_stats('generate_security_code_for_number', False)
                return jsonify({'success': False, 'error': '该手机号已有有效的安全码'}), 400
...
def check_security_code_expiry():
    ...
    try:
        ...
        if not result:
            return jsonify({'success': False, 'error': '手机号不存在'}), 404
        if result['is_used']:
            return jsonify({'success': False, 'error': '该手机号已被使用', 'is_expired': True, 'can_regenerate': False}), 200
        if not result['security_code']:
            return jsonify({'success': True, 'is_valid': False, 'is_expired': True, 'can_regenerate': True}), 200
        now = datetime.now()
...
def verify_and_copy():
    ...
    try:
        ...
        if not captcha_ticket or not captcha_randstr:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '请先完成人机验证'}), 400
        if not phone_number or len(phone_number) != 11:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '手机号格式不正确'}), 400
        if not security_code or len(security_code) != 6:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '安全码格式不正确'}), 400
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute(
        ...
        if not result['security_code']:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '请先生成安全码'}), 400
        now = datetime.now()
        expires_at = parse_datetime(result['security_code_expires_at'])
        if expires_at and now > expires_at:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '安全码已过期，请重新生成'}), 400
        if result['client_ip'] != client_ip:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '安全码只能在生成时的客户端下使用'}), 403
        if result['security_code'] != security_code:
            update_api_stats('verify_and_copy', False)
            return jsonify({'success': False, 'error': '安全码错误'}), 400
        cursor.execute(
        ...
        return jsonify({'success': True, 'phone_number': phone_number, 'message': '验证成功，可以复制完整号码'})
    except Exception as e:
        update_api_stats('verify_and_copy', False)
        return jsonify({'success': False, 'error': str(e)}), 500
    finally:
    ...
    try:
        ...
        with QUOTA_LOCK:
            ...
            for ip, count in sorted_users:
                top_users.append({'ip': ip, 'count': count, 'is_current': ip == client_ip})
        return jsonify({
            'success': True,
            'quota': {
...
def get_all_quota():
    try:
        ...
        return jsonify({'success': True, 'quota_data': quota_data, 'summary': summary, 'overall_stats': overall_stats})
    except Exception as e:
...
def download_quota_data():
    try:
        ...
        try:
            ...
            return jsonify({'success': False, 'error': 'openpyxl 库未安装，无法生成Excel文件'}), 500
    except Exception as e:
        update_api_stats('download_quota_data', False)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_client_info():
    ...
    try:
        ...
        conn.close()
        return jsonify({'success': True, 'client_ip': client_ip, 'stats': {'total_generated': total_generated, 'total_used': total_used, 'available': total_generated - total_used}})
    except Exception as e:
        update_api_stats('get_client_info', False)
        return jsonify({'success': False, 'error': str(e)}), 500
